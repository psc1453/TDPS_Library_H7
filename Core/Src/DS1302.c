...
extern uint32_t b_date, b_month, b_year, b_hour, b_minute, b_second, b_day;

void DS1302_Init(DS1302* ds1302)
{
...
void DS1302_writeOut(DS1302* ds1302, const uint8_t value, uint8_t readAfter)
{
    ...
    GPIO_InitStruct.Pin = ds1302->DS1302_SDA_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(ds1302->DS1302_SDA_Port, &GPIO_InitStruct);

    for (i = 0; i < 8; i++) {
        //digitalWrite(io_pin_, (value >> i) & 1);
        if( (value >> i) & 1 ) {
            HAL_GPIO_WritePin(ds1302->DS1302_SDA_Port, ds1302->DS1302_SDA_Pin, GPIO_PIN_SET);
        } else {
            HAL_GPIO_WritePin(ds1302->DS1302_SDA_Port, ds1302->DS1302_SDA_Pin, GPIO_PIN_RESET);
        }
        //HAL_Delay(1);
        HAL_GPIO_WritePin(ds1302->DS1302_SCL_Port, ds1302->DS1302_SCL_Pin, GPIO_PIN_RESET);    //digitalWrite(sclk_pin_, HIGH);
        HAL_Delay(2);
        ...
        if (readAfter && i == 7) {
            ...
            HAL_GPIO_WritePin(ds1302->DS1302_SCL_Port, ds1302->DS1302_SCL_Pin, GPIO_PIN_SET);
            HAL_Delay(1);
            HAL_GPIO_WritePin(ds1302->DS1302_SDA_Port, ds1302->DS1302_SDA_Pin, GPIO_PIN_RESET);
            GPIO_InitStruct.Pin = ds1302->DS1302_SDA_Pin;
            ...
            HAL_GPIO_WritePin(ds1302->DS1302_SCL_Port, ds1302->DS1302_SCL_Pin, GPIO_PIN_SET);  //digitalWrite(sclk_pin_, LOW);
...
uint8_t DS1302_readIn(DS1302* ds1302)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        HAL_GPIO_WritePin(ds1302->DS1302_SCL_Port, ds1302->DS1302_SCL_Pin, GPIO_PIN_SET);    //digitalWrite(sclk_pin_, HIGH);
        HAL_Delay(2);                                                                //delayMicroseconds(1);
        HAL_GPIO_WritePin(ds1302->DS1302_SCL_Port, ds1302->DS1302_SCL_Pin, GPIO_PIN_RESET);  //digitalWrite(sclk_pin_, LOW);
        HAL_Delay(1);                                                                //delayMicroseconds(1);
        bit = HAL_GPIO_ReadPin(ds1302->DS1302_SDA_Port, ds1302->DS1302_SDA_Pin);                  //bit = digitalRead(io_pin_);
        HAL_Delay(1);
...
uint8_t DS1302_readRegister(DS1302* ds1302, const uint8_t reg)
{
    ...
    HAL_GPIO_WritePin(ds1302->DS1302_CE_Port, ds1302->DS1302_CE_Pin, GPIO_PIN_SET);
    ...
    HAL_Delay(1);
    HAL_GPIO_WritePin(ds1302->DS1302_CE_Port, ds1302->DS1302_CE_Pin, GPIO_PIN_RESET);
...
void DS1302_writeRegister(DS1302* ds1302, const uint8_t reg, const uint8_t value)
{
    ...
    HAL_GPIO_WritePin(ds1302->DS1302_SCL_Port, ds1302->DS1302_SCL_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(ds1302->DS1302_CE_Port, ds1302->DS1302_CE_Pin, GPIO_PIN_RESET);
}
